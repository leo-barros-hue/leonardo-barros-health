...
import { Plus, Trash2, GripVertical, Copy, ChevronDown, ChevronUp, ImageIcon, LayoutList, ArrowLeft, Save } from "lucide-react";
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-0.5">
              {isEditing ? "Modifique as perguntas e configurações" : "Configure o nome, blocos e perguntas"}
            </p>
          </div>
        </div>
        <Button onClick={handleSave} disabled={saving} className="gap-2">
          <Save className="w-4 h-4" />
          {saving ? "Salvando..." : isEditing ? "Salvar Alterações" : "Criar Formulário"}
        </Button>
      </div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">
          Perguntas ({questions.filter(q => q.question_type !== "section").length})
        </h3>
      ...
      {/* Questions list - full page scrolling */}
      ...
      <div className="flex gap-2 justify-center pt-4">
        <Button variant="outline" onClick={addSection} className="gap-1.5">
          <LayoutList className="w-4 h-4" /> Adicionar Bloco
        </Button>
