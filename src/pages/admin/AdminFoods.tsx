...
const AdminFoods = () => {
  ...
  const startEdit = (food: Food) => {
    ...
    setEditData({
      ...
      protein_per_unit: food.protein_per_unit, carbs_per_unit: food.carbs_per_unit,
      fat_per_unit: food.fat_per_unit, kcal_per_unit: food.kcal_per_unit, fiber_per_unit: food.fiber_per_unit,
    });
  };
      ...
      {showForm && (
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-9 gap-3">
            ...
            <Input type="number" placeholder="Kcal" min={0} step={0.01} value={newFood.kcal_per_unit || ""} onChange={(e) => setNewFood((p) => ({ ...p, kcal_per_unit: Number(e.target.value) }))} className="bg-secondary/50" />
            ...
            <Input type="number" placeholder="Fib" min={0} step={0.01} value={newFood.fiber_per_unit || ""} onChange={(e) => setNewFood((p) => ({ ...p, fiber_per_unit: Number(e.target.value) }))} className="bg-secondary/50" />
          </div>
                  ...
                  <tr key={food.id} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    {editingId === food.id ? (
                      <>
                        ...
                        <td className="py-2 px-2"><Input type="number" min={0} step={0.01} value={editData.kcal_per_unit || ""} onChange={(e) => setEditData((p) => ({ ...p, kcal_per_unit: Number(e.target.value) }))} className="h-8 text-sm text-center bg-secondary/50" /></td>
                        ...
                        <td className="py-2 px-2"><Input type="number" min={0} step={0.01} value={editData.fiber_per_unit || ""} onChange={(e) => setEditData((p) => ({ ...p, fiber_per_unit: Number(e.target.value) }))} className="h-8 text-sm text-center bg-secondary/50" /></td>
