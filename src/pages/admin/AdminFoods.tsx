...
import { Plus, Trash2, Search, Apple, Edit2, Check, X } from "lucide-react";
...
const AdminFoods = () => {
  ...
  const handleAdd = async () => {
    ...
    if (error) {
      ...
      setNewFood({ name: "", category: "Outros", measure: "g", protein_per_unit: 0, carbs_per_unit: 0, fat_per_unit: 0 });
      setShowForm(false);
      fetchFoods();
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Gerencie a base de alimentos com informações nutricionais
          </p>
        </div>
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-7 gap-3">
            <Input
              ...
              className="md:col-span-2 bg-secondary/50"
            />
            ...
            <Select value={newFood.measure} onValueChange={(v) => setNewFood((p) => ({ ...p, measure: v }))}>
              <SelectTrigger className="bg-secondary/50"><SelectValue /></SelectTrigger>
              <SelectContent>
            ...
            <Input
              type="number" placeholder="Ptn" min={0} step={0.01}
              value={newFood.protein_per_unit || ""}
              onChange={(e) => setNewFood((p) => ({ ...p, protein_per_unit: Number(e.target.value) }))}
              className="bg-secondary/50 text-success"
            />
            <Input
              type="number" placeholder="Carb" min={0} step={0.01}
              value={newFood.carbs_per_unit || ""}
              onChange={(e) => setNewFood((p) => ({ ...p, carbs_per_unit: Number(e.target.value) }))}
              className="bg-secondary/50 text-warning"
            />
            <Input
              type="number" placeholder="Fat" min={0} step={0.01}
              value={newFood.fat_per_unit || ""}
              onChange={(e) => setNewFood((p) => ({ ...p, fat_per_unit: Number(e.target.value) }))}
              className="bg-secondary/50 text-destructive"
            />
          </div>
          <p className="text-xs text-muted-foreground">
            Para alimentos em g/ml, insira o valor por 1g ou 1ml. Para unidades, insira o valor por 1 unidade.
          </p>
          <div className="flex gap-2">
            <Button onClick={handleAdd} size="sm" className="gap-1">
              <Check className="w-4 h-4" /> Cadastrar
            </Button>
            <Button onClick={() => setShowForm(false)} size="sm" variant="ghost" className="gap-1">
                ...
                filtered.map((food) => (
                  <tr key={food.id} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="py-3 px-3 text-center">
                      <span className="text-xs px-2 py-1 rounded-full bg-secondary text-muted-foreground">{food.category}</span>
                    </td>
                    ...
                    <td className="py-3 px-1">
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(food.id)} className="h-7 w-7 text-muted-foreground hover:text-destructive">
