...
const emptyStr = { name: "", category: "Outros", measure: "g", protein_per_unit: "", carbs_per_unit: "", fat_per_unit: "", kcal_per_unit: "", fiber_per_unit: "" };

const parseNum = (v: string) => { const n = parseFloat(v.replace(",", ".")); return isNaN(n) ? 0 : n; };
const isValidDecimal = (v: string) => v === "" || /^[\d]*[.,]?[\d]*$/.test(v);
const toDbObj = (s: typeof emptyStr) => ({
  name: s.name, category: s.category, measure: s.measure,
  protein_per_unit: parseNum(s.protein_per_unit), carbs_per_unit: parseNum(s.carbs_per_unit),
  fat_per_unit: parseNum(s.fat_per_unit), kcal_per_unit: parseNum(s.kcal_per_unit), fiber_per_unit: parseNum(s.fiber_per_unit),
});

const DecimalInput = ({ value, onChange, className, placeholder }: { value: string; onChange: (v: string) => void; className?: string; placeholder?: string }) => (
  <Input
    type="text"
    inputMode="decimal"
    value={value}
    onChange={(e) => { if (isValidDecimal(e.target.value)) onChange(e.target.value); }}
    className={className}
...
const AdminFoods = () => {
  ...
  const [newFood, setNewFood] = useState({ ...emptyStr });
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editData, setEditData] = useState({ ...emptyStr });
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from("foods").insert([toDbObj(newFood)]);
    if (error) toast.error("Erro ao cadastrar");
    else { toast.success("Alimento cadastrado!"); setNewFood({ ...emptyStr }); setShowForm(false); fetchFoods(); }
  };
  ...
  const startEdit = (food: Food) => {
    ...
    setEditData({
      name: food.name, category: food.category, measure: food.measure,
      protein_per_unit: String(food.protein_per_unit), carbs_per_unit: String(food.carbs_per_unit),
      fat_per_unit: String(food.fat_per_unit), kcal_per_unit: String(food.kcal_per_unit), fiber_per_unit: String(food.fiber_per_unit),
    });
  };

  const handleSaveEdit = async () => {
    if (!editingId) return;
    const { error } = await supabase.from("foods").update(toDbObj(editData)).eq("id", editingId);
  ...
  const nf = (field: string) => (v: string) => setNewFood((p) => ({ ...p, [field]: v }));
  const ef = (field: string) => (v: string) => setEditData((p) => ({ ...p, [field]: v }));
      ...
      {showForm && (
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-9 gap-3">
            ...
            <DecimalInput placeholder="Kcal" value={newFood.kcal_per_unit} onChange={nf("kcal_per_unit")} className="bg-secondary/50" />
            <DecimalInput placeholder="Ptn" value={newFood.protein_per_unit} onChange={nf("protein_per_unit")} className="bg-secondary/50 text-success" />
            <DecimalInput placeholder="Carb" value={newFood.carbs_per_unit} onChange={nf("carbs_per_unit")} className="bg-secondary/50 text-warning" />
            <DecimalInput placeholder="Fat" value={newFood.fat_per_unit} onChange={nf("fat_per_unit")} className="bg-secondary/50 text-destructive" />
            <DecimalInput placeholder="Fib" value={newFood.fiber_per_unit} onChange={nf("fiber_per_unit")} className="bg-secondary/50" />
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/30">
                ...
                <th className="text-center py-3 px-3 text-muted-foreground font-semibold uppercase text-xs tracking-wider w-20">Kcal</th>
                ...
                <th className="text-center py-3 px-3 text-muted-foreground font-semibold uppercase text-xs tracking-wider w-20">Fib</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="py-8 text-center text-muted-foreground">Nenhum alimento encontrado</td></tr>
              ) : (
                filtered.map((food) => (
                  <tr key={food.id} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    {editingId === food.id ? (
                      <>
                        ...
                        <td className="py-2 px-2"><DecimalInput value={editData.kcal_per_unit} onChange={ef("kcal_per_unit")} className="h-8 text-sm text-center bg-secondary/50" /></td>
                        <td className="py-2 px-2"><DecimalInput value={editData.protein_per_unit} onChange={ef("protein_per_unit")} className="h-8 text-sm text-center text-success bg-secondary/50" /></td>
                        <td className="py-2 px-2"><DecimalInput value={editData.carbs_per_unit} onChange={ef("carbs_per_unit")} className="h-8 text-sm text-center text-warning bg-secondary/50" /></td>
                        <td className="py-2 px-2"><DecimalInput value={editData.fat_per_unit} onChange={ef("fat_per_unit")} className="h-8 text-sm text-center text-destructive bg-secondary/50" /></td>
                        <td className="py-2 px-2"><DecimalInput value={editData.fiber_per_unit} onChange={ef("fiber_per_unit")} className="h-8 text-sm text-center bg-secondary/50" /></td>
                      ...
                      <>
                        ...
                        <td className="py-3 px-3 text-center text-muted-foreground font-medium">{food.kcal_per_unit}</td>
                        ...
                        <td className="py-3 px-3 text-center text-muted-foreground font-medium">{food.fiber_per_unit}</td>
