...
import { Plus, Trash2, GripVertical, Copy, ChevronDown, ChevronUp, ImageIcon } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between mt-4">
          <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Perguntas ({questions.length})</h3>
          ...
          <div className="space-y-3 py-2">
            {questions.map((q, idx) => (
              <div
                ...
                className={`border border-border rounded-xl p-4 space-y-3 bg-secondary/20 transition-all ${dragIdx === idx ? "opacity-50" : ""}`}
              >
                {/* Header row */}
                ...
                {expandedIdx === idx && (
                  <div className="space-y-3 pl-6 border-l-2 border-primary/20 ml-3">
                    {/* Description */}
                    <div>
                      <Label className="text-xs">Descrição/Explicação</Label>
                      <Input value={q.description} onChange={(e) => updateQuestion(idx, "description", e.target.value)} placeholder="Adicionar explicação..." className="mt-1 bg-background h-8 text-sm" />
                    </div>
